...
POST /api/v1/categories/init-defaults # Инициализация предустановленных категорий (Admin)

# WebSocket 
WS   /ws/tickets/{ticket_id}         # Real-time обновления тикета
WS   /ws/status                      # Статус соединения и уведомления
```
...
- [x] **v1.1** - ✅ **ЗАВЕРШЁН (100%)** - категории, приоритеты, файловые вложения, расширенные уведомления
- [ ] **v1.2** - 🚀 **ГОТОВ К СТАРТУ** - дизайн-система, автоприоритизация, SLA-таймеры
- [ ] **v1.3** - Интеграции с внешними системами
- [ ] **v2.0** - White-label решение для продажи

**🎉 СТАТУС:** v1.1 полностью готов к production! Система включает категории, приоритеты, файловые вложения и расширенные уведомления. ngrok настроен и работает.
